...
from CNN_recurrent_new import one_step_rnn
...
DiscRNNOutAll = collections.namedtuple(
    "DiscRNNOutAll",
    ["d_all", "d_all_logits", "c_state", "h_state"])
...
class _PatchDiscriminatorCompareGANImpl(abstract_arch.AbstractDiscriminator):
  ...
  def apply(self, x):
    ...
    latent = tf.image.resize_images(latent, [x_shape[1], x_shape[2]],
...
class _DiscriminatorRNN(abstract_arch.AbstractDiscriminator):
  """PatchDiscriminator architecture.
  ...
  def __init__(self,
    ...
    super(_DiscriminatorRNN, self).__init__(
        name, batch_norm_fn=None, layer_norm=False, spectral_norm=True)
  ...
  def apply(self, x):
    ...
    if not isinstance(x, tuple) or len(x) != 4:
      raise ValueError("Expected 4-tuple, got {}".format(x))
    x, latent, state_c, state_h = x
    x_shape = tf.shape(x)
    #
    # Upscale and fuse latent.
    latent = arch_ops.conv2d(latent, 12, 3, 3, 1, 1,
    ...
    net, state_c, state_h = one_step_rnn(net, state_c, state_h, 256, 256,
                                         self._num_filters_base, scale=2, kernal=[k, k], act=tf.tanh,
                                         spectralnorm=self._spectral_norm)

    num_filters = self._num_filters_base
    for i in range(self._num_layers - 1):
      num_filters = min(num_filters * 2, 512)
      net = arch_ops.conv2d(net, num_filters, k, k, 2, 2,
                            name="d_conv_" + str(i), use_sn=self._spectral_norm)
      net = arch_ops.lrelu(net, leak=0.2)
    ...
    return DiscRNNOutAll(out, out_logits, state_c, state_h)


class _PatchDiscriminatorNolatent(abstract_arch.AbstractDiscriminator):
  """PatchDiscriminator architecture.
  ...
  def __init__(self,
    ...
    super(_PatchDiscriminatorNolatent, self).__init__(
        name, batch_norm_fn=None, layer_norm=False, spectral_norm=True)
...
class DiscriminatorRNN(_CompareGANLayer):

  def __init__(self):
    super(DiscriminatorRNN, self).__init__(
        name="DiscriminatorRNN",
        compare_gan_cls=_DiscriminatorRNN)


class Discriminator_latent(_CompareGANLayer):
...
class Discriminator_nolatent(_CompareGANLayer):

  def __init__(self):
    super(Discriminator_nolatent, self).__init__(
        name="Discriminator",
        compare_gan_cls=_PatchDiscriminatorNolatent)
...
class ResCNN(tf.keras.Sequential):
  """ResCNN architecture."""
  ...
  def __init__(self,
    ...
    head = [
        tf.keras.layers.Conv2D(filters=num_filters_base, kernel_size=3, padding="same"),
        ChannelNorm(),
        tf.keras.layers.ReLU()
    ]
    ...
    super(ResCNN, self).__init__(layers=model, name=name)
