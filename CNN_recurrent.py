...
def one_step_rnn(tensor, state_c, state_h, Height, Width, num_filters, scale, kernal, act):
    ...
    cell = rnn_cell.ConvLSTMCell(shape=[Height // scale, Width // scale], activation=act,
...
def MV_analysis_nr(tensor, num_filters, out_filters):
  """Builds the analysis transform."""
...
def MV_synthesis_nr(tensor, num_filters):
  """Builds the synthesis transform."""
...
def Res_analysis_nr(tensor, num_filters, out_filters):
  """Builds the analysis transform."""
...
def Res_synthesis_nr(tensor, num_filters):
  """Builds the synthesis transform."""
...
def rec_density_new(tensor, num_filters, Height, Width, c_state, h_state, k=3, act=tf.tanh):
